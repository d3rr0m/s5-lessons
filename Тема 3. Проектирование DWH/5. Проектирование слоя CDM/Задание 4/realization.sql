...
alter table cdm.dm_settlement_report add constraint dm_settlement_report_orders_orders_bonus_payment_sum_check check(orders_bonus_payment_sum >= (0)::numeric);
alter table cdm.dm_settlement_report add constraint dm_settlement_report_orders_orders_bonus_granted_sum_check check(orders_bonus_granted_sum >= (0)::numeric);
alter table cdm.dm_settlement_report add constraint dm_settlement_report_orders_order_processing_fee_check check(order_processing_fee >= (0)::numeric);
alter table cdm.dm_settlement_report add constraint dm_settlement_report_orders_restaurant_reward_sum_check check(restaurant_reward_sum >= (0)::numeric);
